...
export const command = {
    ...
    async execute(interaction: ChatInputCommandInteraction | ButtonInteraction) {
        ...
        let upnext =
            queue!.size > 10
                ? queue?.tracks
                      .filter((track, index) => index >= 0 && index <= 10)
                      .map((track, index) => `${index + 1}. ${track.author} - ${track.title}`)
                      .join('\n') +
                  '\n...\n + `' +
                  (queue?.tracks.size! - 10) +
                  ' more tracks`'
                : queue?.tracks.map((track, index) => `${index + 1}. ${track.author} - ${track.title}`).join('\n');
        if (!queue) return await interaction.reply({ content: 'There is no music playing!', ephemeral: true });
        let queueEmbed = new EmbedBuilder()
            .setTitle(`Queue for ${interaction.guild?.name}`)
            .setColor(0xf21f52)
            .setThumbnail(interaction.guild?.iconURL() as string)
            .addFields({ name: 'Current Track', value: `${queue.currentTrack?.author} - ${queue.currentTrack?.title}` }, { name: 'Up Next', value: upnext ? upnext : 'No tracks in queue' });
