...
export const playerEvents = (player: Player) => {
    ...
    player.events.on('playerStart', async (queue, track) => {
        ...
        let embed = await metadata.channel.send({ embeds: nowPlayingEmbed(queue, track), components: nowPlayingComponents(queue) }).catch((e: Error) => {
            ...
            channel.send({
                ...
                embeds: [
                    new EmbedBuilder()
                        .setAuthor({ name: player.client.user!.username, iconURL: player.client.user!.avatarURL() as string, url: `https://discord.com/users/${player.client.user?.id}` })
    ...
    player.events.on('playerError', (queue, error, track) => {
        let metadata = queue.metadata as any;
        let errorChannel = queue.player.client.guilds.cache.get(WGServers.main)?.channels.cache.get(WGChannels.botLog) as TextChannel;
    ...
    return [
        new EmbedBuilder()
            ...
            .setDescription(
                ...
                })}. Requested by ${track.requestedBy}\n${queue.node.isPlaying() ? ':arrow_forward:' : ':pause_button:'} ${slidebar}\n\nUseful Commands: </play:${WGCommands.play}> </playnext:${WGCommands.playNext}> </lyrics:${
                    WGCommands.lyrics
                }> </queue:${WGCommands.queue}> `
            )
            .setColor(0xf21f52)
...
export function nowPlayingComponents(queue: GuildQueue) {
    ...
    return [
        new ActionRowBuilder().addComponents(
            ...
            new ButtonBuilder().setEmoji(loopButton.emoji).setStyle(ButtonStyle.Secondary).setCustomId(loopButton.action)
        )
    ];
}
